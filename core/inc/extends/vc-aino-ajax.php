    ...
    public static function aino_change_rate() {
        ...
        foreach ($session['rates'] as $rate) {
            if ($rate->method_id == filter_input(INPUT_POST, 'shipping_checkbox')) {
                ...
                // Calculate taxes to shipping prices and totals only if enabled in WooCommerce
                $taxes = get_option('woocommerce_calc_taxes')=='yes' ? $vc_aino_heplers->get_new_tax($cost, array_keys(WC()->cart->shipping_taxes)) : 0;
        ...
        if ($id) {
            ...
            // Add taxes to shipping prices and totals only if enabled in WooCommerce
            if(get_option('woocommerce_calc_taxes')=='yes'){
                WC()->shipping->shipping_taxes = $taxes;    
                $session['rates'][$id]->taxes = $taxes;
            }
